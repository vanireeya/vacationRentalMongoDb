...
class ProfileTraveller extends Component {
    ...
    componentDidMount() {
        ...
        axios.get(`${ROOT_URL}/profile/` + this.state.myData.uid ,{headers: {Authorization:  this.state.myData.token} })
            .then((response) => {
    ...
    uploadImage() {
        ...
        this.props.onSubmitHandleProfile(formData)
            .then((result) => {
                ...
                if (result && result.data.status == 1) {
                    ...
                    this.setState({
                        imageView:  result.data.info.fileName
        ...
        return (
            <div>
                ...
                <div style={{ "text-align": "center", marginTop: "2%" }}>
                    ...
                    <div>
                        <h2><b>{this.props.myData.firstname + " " + this.props.myData.lastname}</b></h2>
...
const mapDispatchStateToProps = dispatch => {
    return {
        onSubmitHandle: (data) => {
            return axios.post(`${ROOT_URL}/updateProfile`, data,{ withCredentials: true,headers: {   Authorization:  data.token} } )            
                .then(response => {
                    if (response.data.status == 1) {
                        ...
                        dispatch({ type: 'UPDATEMYDATA', payload: res });
                        return response;
                    } else {
                        return response;
                    }
        ...
        onSubmitHandleProfile: (formData) => {
        
        return axios.post(`${ROOT_URL}/updateProfilePic`, formData,{ withCredentials: true,headers: {   Authorization:  formData.get("token")} })
                .then(response => {
                    if (response.data.status == 1) {
                        ...
                        let res = {
                            ...
                            data: {
                                ...
                                profileImage:response.data.info.fileName,
                                // lastname: data.lastname,
                                
                            }
                        }
                        dispatch({ type: 'PROFILEIMAGEMYDATA', payload: res });
                        return response;
                    } else {
                        return response;
                    }
...
export default connect(mapStateToProps, mapDispatchStateToProps)(ProfileTraveller);
