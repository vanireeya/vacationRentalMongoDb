...
class MytripTraveller extends Component {
    ...
    componentDidMount() {
        if (this.state.myData) {
            ...
            axios.get(`${ROOT_URL}/getTrips/` + data, { headers: { Authorization: this.state.myData.token } })
                .then((response) => {
    ...
    render() {
        ...
        const renderPageNumbers = this.state.displayPages.map(number => {
            return (
                ...
                <li>
                    <a onClick={() => { this.getNext(number) }} href="#">
                        {number}
                    </a>
                </li>
                // <li
...
export default connect(mapStateToProps)(MytripTraveller);
